...
#include "oblivious_transfer_short.tpp"
...
TEST_CASE("Oblivious Transfer Interface (short message)", "[libOTe]") {
    const int n = 4;
    ...
    auto thrd = std::thread([&] {
        CHECK_NOTHROW(TwoChooseOne_Sender_Short<IknpOtExtSender, IknpOtExtReceiver, bitLength, n>(ip, content));
    });

    std::bitset<n> choice(0b1010);
    std::array<Element, n> ret;
    CHECK_NOTHROW(ret = TwoChooseOne_Receiver_Short<IknpOtExtSender, IknpOtExtReceiver, bitLength, n>(ip, choice));
    thrd.join();
